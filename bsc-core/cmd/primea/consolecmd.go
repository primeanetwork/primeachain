...
var (
	...
	consoleCommand = &cli.Command{
		...
		Description: `
The Primea console is an interactive shell for the JavaScript runtime environment
...
which exposes a node admin interface as well as the Ðapp JavaScript API.
See https://geth.ethereum.org/docs/interacting-with-geth/javascript-console.
This command allows to open a console on a running primea node.`,
	}
...
// localConsole starts a new primea node, attaching a JavaScript console to it at the
// same time.
func localConsole(ctx *cli.Context) error {
	// Create and start the node based on the CLI flags
	prepare(ctx)
	stack, backend := makeFullPrimeaNode(ctx)
...
// remoteConsole will connect to a remote primea instance, attaching a JavaScript
// console to it.
func remoteConsole(ctx *cli.Context) error {
	...
	if err != nil {
		utils.Fatalf("Unable to attach to remote primea: %v", err)
	}
	config := console.Config{
		DataDir: utils.MakeDataDir(ctx),
		DocRoot: ctx.String(utils.JSpathFlag.Name),
...
// ephemeralConsole starts a new primea node, attaches an ephemeral JavaScript
...
primea --exec "%s" console`, b.String())
	return nil
}
